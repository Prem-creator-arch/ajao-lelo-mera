...
class CameraViewer:
    def __init__(self):
        ...
        print(f"[CAM2] Subscribing to active image topic: {topic}")
        self.node.subscribe(Image, topic, self.image_callback)

    def find_camera_topic(self):
        return "/iris/camera_forward/image_raw" 
    def image_callback(self, msg: Image):
        ...
        cv2.imshow("Front Camera (CAM2)", frame)
        cv2.waitKey(1)

if __name__ == "__main__":
    print("[CAM2] Starting Zero-Latency Camera Listener...")
    viewer = CameraViewer()
    try:
        while True:
            time.sleep(0.01)
